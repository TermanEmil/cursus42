...
FragTrap::FragTrap (std::string const & name) :
	_lvl(1), _hp(100), _maxHP(100), _eneryPoints(50), _maxEP(50),
	_meleDmg(20), _rangedDmg(15), _armor(5), _name(name)
{
...
FragTrap::FragTrap (u_i const lvl, u_i const hp, u_i const maxHp, u_i const ep,
	u_i const maxEP, u_i const meleDmg, u_i const rangedDmg,
	u_i const armor, std::string const name) : 
	_lvl(lvl), _maxHP(maxHp), _maxEP(maxEP), _meleDmg(meleDmg),
	_rangedDmg(rangedDmg), _armor(armor), _name(name)
{
	setHP(hp);
	setEP(ep);
}
...
void FragTrap::takeDamage (unsigned int const amount) {
	...
	std::cout << "FR4G-TP " << _name << " takes " << armoredAmount <<
...
void FragTrap::beRepaired (unsigned int const amount) {
	...
	std::cout << "FR4G-TP " << _name << " gets repaired with " <<
